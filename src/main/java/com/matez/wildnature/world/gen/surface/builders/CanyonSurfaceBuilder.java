...
public class CanyonSurfaceBuilder extends SurfaceBuilder<CanyonSurfaceBuilderConfig> {
    private final static RidgedMulti ridgedMultiNoise = new RidgedMulti();
    private final static Perlin heightNoise = new Perlin();

    public CanyonSurfaceBuilder(Function<Dynamic<?>, ? extends CanyonSurfaceBuilderConfig> p_i51312_1_) {
        super(p_i51312_1_);
        ...
        setRegistryName("wildnature", "canyon_surface_builder");
    }
    ...
    public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, CanyonSurfaceBuilderConfig config) {
        ...
        if (vnoise >= 0.50 && vnoise <= 2) {
            ...
            if (vnoise >= 0.53) {
                ...
                int height2 = getNoiseHeight(vnoise, 0.53, 0.625, 5, 25, 17, 0.63, random, seed, pos);
            ...
            if (vnoise >= 0.57) {
                BlockPos pos2 = new BlockPos(x, seaLevel + 10 + hvar, z);
                int height2 = getNoiseHeight(vnoise, 0.57, 0.625, 1, 5, 17, 0.63, random, seed, pos);
                realY2 = realY2 - height2;
                int startPointY2 = -height2 / 2;//pos.y - startPointY -> block being set
                for (int a = pos2.getY() - startPointY2; a > pos2.getY() - height2; a--) {
                    if (a >= seaLevel) {
            ...
            if (vnoise >= 0.59) {
                BlockPos pos2 = new BlockPos(x, seaLevel + 5 + hvar, z);
                int height2 = getNoiseHeight(vnoise, 0.59, 0.625, 1, 8, 17, 0.63, random, seed, pos);
            ...
            if (vnoise >= 0.6) {
                ...
                int height2 = getNoiseHeight(vnoise, 0.6, 0.625, 0, 10, 17, 0.63, random, seed, pos);
