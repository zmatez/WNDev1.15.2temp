...
public class OpenSimplex2S {
	...
	/**
	 * 2D SuperSimplex noise with Derivatives, standard lattice orientation.
	 */
	 ...
	 * Probably slightly less optimal for heightmaps or continent maps.
	 */
	/*public void noise2_XBeforeY(double x, double y, Values2D1 output) {
	...
	private void noise2_Base(double xs, double ys, Values2D1 output) {
		output.value = output.dx = output.dy = 0;
		...
		for (int i = 0; i < 4; i++) {
			...
			output.dx += (grad.dx * attn - 8 * dx * extrapolation) * attnSq * attn;
			output.dy += (grad.dy * attn - 8 * dy * extrapolation) * attnSq * attn;
		}
	}
	...
	public void noise3_Classic(double x, double y, double z, Values3D1 output) {
		...
		// Evaluate both lattices to form a BCC lattice.
		noise3_BCC(xr, yr, zr, output, GRADIENTS_3D_CLASSIC);
	}
	...
	public void noise3_XYBeforeZ(double x, double y, double z, Values3D1 output) {
		...
		// Evaluate both lattices to form a BCC lattice.
		noise3_BCC(xr, yr, zr, output, GRADIENTS_3D_XY_BEFORE_Z);
	}
	...
	public void noise3_XZBeforeY(double x, double y, double z, Values3D1 output) {
		...
		// Evaluate both lattices to form a BCC lattice.
		noise3_BCC(xr, yr, zr, output, GRADIENTS_3D_XZ_BEFORE_Y);
	}
	...
	private void noise3_BCC(double xr, double yr, double zr, Values3D1 output, Grad3[] derivativeGrads) {
		output.value = output.dx = output.dy = output.dz = 0;
		...
		while (c != null) {
			...
			if (attn < 0) {
				...
				int pxm = (xrb + c.xrv) & PMASK, pym = (yrb + c.yrv) & PMASK, pzm = (zrb + c.zrv) & PMASK;
				int zPermXY = perm[perm[pxm] ^ pym] ^ pzm;
				Grad3 grad = permGrad3[zPermXY];
				double extrapolation = grad.dx * dxr + grad.dy * dyr + grad.dz * dzr;
				Grad3 dGrad = derivativeGrads[perm[zPermXY]];
				
				double attnSq = attn * attn;
				output.value += attnSq * attnSq * extrapolation;
				output.dx += (dGrad.dx * attn - 8 * dxr * extrapolation) * attnSq * attn;
				output.dy += (dGrad.dy * attn - 8 * dyr * extrapolation) * attnSq * attn;
				output.dz += (dGrad.dz * attn - 8 * dzr * extrapolation) * attnSq * attn;
				c = c.nextOnSuccess;
			}
		}
	}
	...
	/*
	 * Value/Derivative Output Schemes
	 */
