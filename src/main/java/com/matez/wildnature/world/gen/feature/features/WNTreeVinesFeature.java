...
public class WNTreeVinesFeature extends Feature<BlockFeatureConfig> {
   private static final Direction[] DIRECTIONS = Direction.values();

   public WNTreeVinesFeature(Function<Dynamic<?>, ? extends BlockFeatureConfig> p_i51418_1_) {
      super(p_i51418_1_);
      setRegistryName("wildnature","tree_vines_feature");
   }
         ...
         if (worldIn.isAirBlock(mutable)) {
            for (Direction direction : DIRECTIONS) {
               if (direction != Direction.DOWN && direction != Direction.UP) {
                  if (BlockTags.LEAVES.contains(worldIn.getBlockState(mutable.offset(direction)).getBlock())) {
