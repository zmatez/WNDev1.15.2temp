...
public class LettucePlant extends CropBase {
    ...
    public LettucePlant(Properties properties, ResourceLocation regName) {
        super(properties, null, regName);
    }
    @Override
    protected IItemProvider getSeedsItem() {
        return WNItems.LETTUCE;
    }
    @Override
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(state.get(AGE)==4){
            ...
            spawnAsEntity(worldIn,pos,new ItemStack(WNItems.LETTUCE, Utilities.rint(1,2)));
    ...
    public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
        ...
        if(list.isEmpty() && !silkTouch){
            if(state.get(AGE)==4) {
                list.add(new ItemStack(WNItems.LETTUCE, Utilities.rint(1, 2)));
            }
        }
