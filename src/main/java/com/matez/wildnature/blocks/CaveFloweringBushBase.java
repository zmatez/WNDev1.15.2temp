...
public class CaveFloweringBushBase extends CaveBush implements IGrowable {

    public static final BooleanProperty FLOWERING = BooleanProperty.create("flowering");
    public CaveFloweringBushBase(Properties properties, Item.Properties builder, ResourceLocation regName, boolean reversed) {
        super(properties.sound(SoundType.PLANT),builder,regName,reversed);
    ...
    public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
        if(state.getBlock() instanceof CaveFloweringBushBase && state.get(FLOWERING) && CommonConfig.flowerDisappearsOnWalk.get() && !(entityIn instanceof ItemEntity)){
