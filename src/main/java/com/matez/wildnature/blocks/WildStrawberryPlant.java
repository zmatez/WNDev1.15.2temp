...
public class WildStrawberryPlant extends CropBase {
    ...
    public WildStrawberryPlant(Properties properties, Item.Properties builder, ResourceLocation regName) {
    ...
    protected IItemProvider getSeedsItem() {
        return WNBlocks.BUSH_WILD_STRAWBERRY;
    }
    @Override
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(state.get(AGE)==5){
            ...
            spawnAsEntity(worldIn,pos,new ItemStack(WNItems.WILD_STRAWBERRY, Utilities.rint(1,2)));
    ...
    public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
        ...
        if(list.isEmpty() && !silkTouch){
            if(state.get(AGE)==5) {
                list.add(new ItemStack(WNItems.WILD_STRAWBERRY, Utilities.rint(1, 2)));
            }
        }
