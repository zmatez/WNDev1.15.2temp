...
public class WildBlueberryPlant extends CropBase {
    ...
    public WildBlueberryPlant(Properties properties, Item.Properties builder, ResourceLocation regName) {
    ...
    protected IItemProvider getSeedsItem() {
        return WNBlocks.BUSH_WILD_BLUEBERRY;
    }
    @Override
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(state.get(AGE)==getMaxAge()){
            ...
            spawnAsEntity(worldIn,pos,new ItemStack(WNItems.BLUEBERRY, Utilities.rint(1,2)));
    ...
    public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
        ...
        if(list.isEmpty() && !silkTouch){
            if(state.get(AGE)==getMaxAge()) {
                list.add(new ItemStack(WNItems.BLUEBERRY, Utilities.rint(1, 2)));
            }
        }
