...
public class CustomPistonTileEntity extends TileEntity implements ITickableTileEntity {
   private BlockState pistonState;
   private Block head,moving;
   ...
   private BlockState getCollisionRelatedBlockState() {
      return !this.isExtending() && this.shouldPistonHeadBeRendered() && this.pistonState.getBlock() instanceof CustomPistonBlock ? head.getDefaultState().with(CustomPistonHeadBlock.TYPE, PistonType.STICKY).with(CustomPistonHeadBlock.FACING, this.pistonState.get(CustomPistonBlock.FACING)) : this.pistonState;
   }
   ...
   public VoxelShape getCollisionShape(IBlockReader p_195508_1_, BlockPos p_195508_2_) {
      ...
      if (!this.extending && this.shouldHeadBeRendered) {
         voxelshape = this.pistonState.with(CustomPistonBlock.EXTENDED, Boolean.valueOf(true)).getCollisionShape(p_195508_1_, p_195508_2_);
      } else {
      ...
      if ((double)this.progress < 1.0D && direction == this.getMotionDirection()) {
         ...
         if (this.shouldPistonHeadBeRendered()) {
            blockstate = head.getDefaultState().with(CustomPistonHeadBlock.FACING, this.pistonFacing).with(CustomPistonHeadBlock.SHORT, Boolean.valueOf(this.extending != 1.0F - this.progress < 4.0F));
         } else {
