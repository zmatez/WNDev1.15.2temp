...
public class InsectFlyingMovementController extends MovementController {
    ...
    public InsectFlyingMovementController(MobEntity p_i225710_1_, int p_i225710_2_, boolean p_i225710_3_) {
        super(p_i225710_1_);
    ...
    public void tick() {
        if (this.action == MovementController.Action.MOVE_TO) {
            ...
            double d0 = this.posX - this.mob.getPosition().getX();
            double d1 = this.posY - this.mob.getPosition().getY();
            double d2 = this.posZ - this.mob.getPosition().getZ();
