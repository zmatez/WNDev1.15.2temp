...
public class DuckPathNavigator extends GroundPathNavigator {
   private boolean shouldAvoidSun;

   public DuckPathNavigator(MobEntity entitylivingIn, World worldIn) {
   ...
   protected Vec3d getEntityPosition() {
      return new Vec3d(this.entity.getPosition().getX(), (double)this.getPathablePosY(), this.entity.getPosition().getZ());
   }
   ...
   private int getPathablePosY() {
      if (this.entity.isInWater() && this.getCanSwim()) {
         ...
         Block block = this.world.getBlockState(new BlockPos(this.entity.getPosition().getX(), (double)i, this.entity.getPosition().getZ())).getBlock();
         int j = 0;

         while(block == Blocks.WATER) {
            ++i;
            block = this.world.getBlockState(new BlockPos(this.entity.getPosition().getX(), (double)i, this.entity.getPosition().getZ())).getBlock();
            ++j;
            if (j > 16) {
   ...
   protected void trimPath() {
      ...
      if (this.shouldAvoidSun) {
         if (this.world.getLight(new BlockPos(this.entity.getPosition().getX(), this.entity.getBoundingBox().minY + 0.5D, this.entity.getPosition().getZ()))==15) {
            return;
         }

         for(int i = 0; i < this.currentPath.getCurrentPathLength(); ++i) {
            PathPoint pathpoint = this.currentPath.getPathPointFromIndex(i);
            if (this.world.getLight(new BlockPos(pathpoint.x, pathpoint.y, pathpoint.z))==15) {
